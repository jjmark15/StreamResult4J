...
public final class ResultGatherers {
    ...
    public static <T, R, E extends Exception> Gatherer<T, ?, StreamResult<R, Cause.Single<E>>> mapFallible(FunctionWithException<T, R, E> mapper, boolean continueAfterFailure) {
        class State {
    ...
    public static <T, R, E1 extends Exception, E2 extends Exception> Gatherer<StreamResult<T, Cause.Single<E1>>, ?, StreamResult<R, Cause.Double<E1, E2>>> mapFallible2(FunctionWithException<T, R, E2> mapper, boolean continueAfterFailure) {
        return mapFallibleN(mapper, Cause.Double::fromSingle, Cause.Double::ofSecond, continueAfterFailure);
    }

    public static <T, R, E1 extends Exception, E2 extends Exception, E3 extends Exception> Gatherer<StreamResult<T, Cause.Double<E1, E2>>, ?, StreamResult<R, Cause.Triple<E1, E2, E3>>> mapFallible3(FunctionWithException<T, R, E3> mapper, boolean continueAfterFailure) {
        return mapFallibleN(mapper, Cause.Triple::fromDouble, Cause.Triple::ofThird, continueAfterFailure);
    }

    private static <T, R, E extends Exception, C1 extends Cause, C2 extends Cause> Gatherer<StreamResult<T, C1>, ?, StreamResult<R, C2>> mapFallibleN(FunctionWithException<T, R, E> mapper, Function<C1, C2> causeUpgrade, Function<E, C2> causeFactory, boolean continueAfterFailure) {
        class State {
            private boolean hasFailed = false;
        }
